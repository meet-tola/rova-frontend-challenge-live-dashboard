...
export function DeliveryGrid({ onIntervene }: DeliveryGridProps) {
  ...
  // Memoize filtered deliveries to prevent recalculation on every render
    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden flex flex-col">
      <div className="overflow-x-auto flex-1">
        <div className="w-full">
          {/* Table Header */}
          <div className="grid grid-cols-[120px_1fr_180px_150px_150px_150px] gap-0 bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
      ...
      <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 text-sm text-slate-600">
        Showing {filteredDeliveries.length} of {filteredDeliveries.length} deliveries
      </div>
    </div>
  );
}
